...
| `reports_api`       | Go API для чтения готовой витрины            | <http://localhost:8000>                        |
...
Первый запуск читает всю 30-дневную тестовую историю до начала текущего часа.
Следующие запуски читают данные от watermark с перекрытием в два часа. Перекрытие
